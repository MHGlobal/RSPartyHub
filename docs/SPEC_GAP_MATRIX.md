...
| Network discovery | S1 §6.2–6.8; S2 AC-017; S4 VW Network/Offline | Bind LAN interface, choose/refresh advertised address and QR; announce a best-effort custom TCP mDNS service without relying on it. | **PARTIAL** | `apps/server/src/{discovery.ts,mdns.ts}`; `apps/server/test/mdns.test.ts`; `docs/NETWORKING.md` | Address-change handling/UI remains. | LAN + hotspot tests with IP-only fallback and mDNS when available. |
